...
define(function() {
  var HBOX_WARNING_REGEX, LATEX_ERROR_REGEX, LATEX_WARNING_REGEX, LINES_REGEX, LOG_WRAP_LIMIT, LatexParser, LogText, PACKAGE_REGEX, PACKAGE_WARNING_REGEX, state;
  LOG_WRAP_LIMIT = 79;
  LATEX_ERROR_REGEX = /^.+\..+:\d+: (.*)$/;
  ...
  (function() {
      ...
      while ((this.currentLine = this.log.nextLine()) !== false) {
        ...
        if (this.state === state.ERROR) {
          if (this.currentLine[0] === '!') {
            ...
            this.currentError.content = this.currentLine.match(/^.+\..+:\d+: (.*)$/)[1];
            this.currentError.message = this.currentError.content;
            this.currentError.raw = this.currentLine;
            lineNo = this.currentError.raw.match(/:([0-9]+):/);
            if (lineNo) {
              this.currentError.line = parseInt(lineNo[1], 10);
            }
            this.currentError.file = this.currentError.raw.match(/^(.+):\d+:/)[1];
            this.data.push(this.currentError);
            this.state = state.NORMAL;
          }
        }
    ...
    this.currentLineIsError = function() {
      return this.currentLine[0] === '!' || this.currentLine.match(LATEX_ERROR_REGEX);
    };
