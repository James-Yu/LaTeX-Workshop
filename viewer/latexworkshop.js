...
socket.addEventListener("message", event => {
  ...
  switch (data.type) {
    case "synctex":
      ...
      const container = document.getElementById("viewerContainer");
      const pos = PDFViewerApplication.pdfViewer._pages[
        data.data.page - 1
      ].viewport.convertToViewportPoint(data.data.x, data.data.y);
    ...
    case "refresh":
      ...
      socket.send(
        JSON.stringify({
          ...
          viewerHistory: {
            history: viewerHistory._history,
            currentIndex: viewerHistory._currentIndex
          }
        })
      );
    ...
    case "params":
      ...
      if (data.invert > 0) {
        document.querySelector("#viewer").style.filter = `invert(${data.invert *
          100}%)`;
...
if (embedded) {
  document.addEventListener("click", e => {
    if (
      e.target.nodeName === "A" &&
      !e.target.href.startsWith(window.location.href)
    ) {
      // is external link
      socket.send(
        JSON.stringify({ type: "external_link", url: e.target.href })
      );
      e.preventDefault();
    }
  });
...
document.addEventListener(
  ...
  evPageRendered => {
    ...
    canvas_dom.onclick = e => {
      ...
      const pos = PDFViewerApplication.pdfViewer._pages[page - 1].getPagePoint(
        left,
        canvas_dom.offsetHeight - top
      );
      socket.send(
        JSON.stringify({
          type: "click",
...
document
  ...
  .addEventListener("click", setHistory);
document
...
window.addEventListener("keydown", evt => {
  ...
  if (evt.keyCode === 70 && evt.target.nodeName !== "INPUT") {
    // ignore F typed in the search box
    showToolbar(false);
...
function showToolbar(animate) {
  ...
  d.className =
    d.className.replace(" hide", "") + (animate ? "" : " notransition");

  hideToolbarInterval = setInterval(() => {
    if (
      !PDFViewerApplication.findBar.opened &&
      !PDFViewerApplication.pdfSidebar.isOpen &&
  ...
  if (trimSelect.selectedIndex <= 0) {
    ...
    for (const page of viewer.getElementsByClassName("page")) {
      for (const layer of page.getElementsByClassName("annotationLayer")) {
        for (const secionOfAnnotation of layer.getElementsByTagName(
          "section"
        )) {
          if (secionOfAnnotation.dataset.originalLeft !== undefined) {
            secionOfAnnotation.style.left =
              secionOfAnnotation.dataset.originalLeft;
          }
        }
      }
    }
...
const trimPage = page => {
  ...
  if (m) {
    ...
    for (const secionOfAnnotation of secionOfAnnotationArray) {
      ...
      if (secionOfAnnotation.dataset.originalLeft === undefined) {
        secionOfAnnotation.setAttribute(
          "data-original-left",
          secionOfAnnotation.style.left
        );
      } else {
