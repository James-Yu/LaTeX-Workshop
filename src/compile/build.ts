...
import { lw } from '../lw'
import type { ProcessEnv, Step } from '../types'
...
function autoBuild(file: string, type: 'onFileChange' | 'onSave',  bibChanged: boolean = false) {
    ...
    if (!bibChanged && lw.root.subfiles.path && configuration.get('latex.rootFile.useSubFile')) {
        return build(true, lw.root.subfiles.path, lw.root.file.langId)
    } else {
...
function canAutoBuild(): boolean {
    ...
    if (Date.now() - lw.compile.lastBuildTime < (configuration.get('latex.autoBuild.interval', 1000) as number)) {
        return false
    }
    return true
}
    ...
    const result: boolean = await new Promise(resolve => {
        ...
        lw.compile.process.on('exit', async (code, signal) => {
            ...
            if (!step.isExternal && signal !== 'SIGTERM' && !step.isRetry && configuration.get('latex.autoBuild.cleanAndRetry.enabled')) {
                ...
                // External command
                logger.log(`Build returns with error: ${code}/${signal} on PID ${lw.compile.process?.pid}.`)
