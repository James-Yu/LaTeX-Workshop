...
function findFromMagic(): string | undefined {
    ...
    if (result) {
        ...
        content = extension.file.read(filePath)
...
function findFromActive(): string | undefined {
    ...
    if (result) {
        ...
        if (rootFilePath) {
            ...
            root.subfiles.langId = extension.file.getLangId(activeFilePath)
            return rootFilePath
        } else {
...
async function findInWorkspace(): Promise<string | undefined> {
    ...
    try {
        ...
        for (const fileUri of fileUris) {
            ...
            const flsChildren = extension.cache.getFlsChildren(fileUri.fsPath)
            ...
            if (result) {
                // Can be a root
                const children = extension.cache.getTeXChildren(fileUri.fsPath, fileUri.fsPath, [])
