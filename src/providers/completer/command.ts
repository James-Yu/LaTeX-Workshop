...
export class Command {
    ...
    initialize (defaultCommands: { [key: string]: AutocompleteEntry }, defaultEnvs: string[]) {
        ...
        Object.keys(envSnippet).forEach(key => {
            ...
            const command = new CommandCompletionItem(
                `\\begin{${item.command}} ... \\end{${item.command}}`,
    ...
    provide () : vscode.CompletionItem[] {
        ...
        if (configuration.get('intellisense.unimathsymbols.enabled')) {
            ...
            suggestions = { ...suggestions, ...this.defaultSymbols }
        }
        ...
        if (vscode.window.activeTextEditor) {
            const items = this.getCommandItems(
                ...
                editor.edit(editBuilder => {
                    ...
                    for (const selection of editor.selections) {
                        ...
                        editBuilder.replace(
                            ...
                            selectedCommand
                                .replace(/(.*)(\${\d.*?})/, `$1${selectedContent}`) // Replace text
    ...
    loadSymbols () {
        const symbols = JSON.parse(
            fs.readFileSync(`${this.extension.extensionRoot}/data/unimathsymbols.json`).toString(),
        )
