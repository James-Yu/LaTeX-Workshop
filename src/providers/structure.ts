...
export class SectionNodeProvider implements vscode.TreeDataProvider<Section> {
    private _onDidChangeTreeData: vscode.EventEmitter<Section | undefined> = new vscode.EventEmitter<
        Section | undefined
    >()
    ...
    buildModel (
        ...
        let pattern =
            '(?:((?:\\\\(?:input|InputIfFileExists|include|subfile|(?:(?:sub)?import\\*?{([^}]*)}))(?:\\[[^\\[\\]\\{\\}]*\\])?){([^}]*)})|((?:\\\\('
        this.hierarchy.forEach((section, index) => {
            pattern += section
        ...
        for (let lineNumber = 0; lineNumber < lines.length; lineNumber++) {
            ...
            if (result && result[1] === 'begin') {
                ...
                const newEnv = new Section(
                    `${env.name.charAt(0).toUpperCase() + env.name.slice(1)}: ${caption}`,
            ...
            if (result && result[5] in this.sectionDepths) {
                ...
                imports &&
                result &&
                (result[1].startsWith('\\input') ||
                    result[1].startsWith('\\InputIfFileExists') ||
                    result[1].startsWith('\\include') ||
                    result[1].startsWith('\\subfile') ||
                    result[1].startsWith('\\subimport') ||
                    result[1].startsWith('\\import'))
            ) {
                // zoom into this file
                // resolve the path
                let inputFilePath
                if (result[1].startsWith('\\subimport')) {
                    ...
                    inputFilePath = utils.resolveFile(
                        [...texDirs, path.dirname(filePath), this.extension.manager.rootDir],
                        result[3],
                    )
                }
    ...
    getCaption (lines: string[], env: { name: string; start: number; end: number }) {
        ...
        const result = /(?:\\caption(?:\[[^\[\]]*\])?){((?:(?:[^\{\}])|(?:\{[^\{\}]*\}))+)}/gms.exec(content)
        if (result) {
