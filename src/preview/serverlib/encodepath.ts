...
export function hasPrefix(url: string) {
    return url.includes(PDF_PREFIX)
}

export function encodePathWithPrefix(pdfFilePath: vscode.Uri) {
    return PDF_PREFIX + encodePath(pdfFilePath.toString(true))
}

export function decodePathWithPrefix(b64urlWithPrefix: string): vscode.Uri {
    const s = b64urlWithPrefix.replace(PDF_PREFIX, '')
