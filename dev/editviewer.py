...
with open(args.web + '/viewer.html', 'rt', encoding='utf-8') as fin:
    with open(args.viewer + '/viewer.html', 'wt', encoding='utf-8') as fout:
        for line in fin:
            line = line \
                    .replace('''<title>PDF.js viewer</title>''', '''<meta http-equiv="Content-Security-Policy" content="default-src 'self'; base-uri 'none'; connect-src 'self' ws://127.0.0.1:*; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:;">\n    <title>PDF.js viewer</title>''') \
    ...
    with open(args.viewer + '/viewer.mjs', 'wt', encoding='utf-8') as fout:
        ...
        for line in fin:
            line = line \
                ...
                .replace(''' eventBus.on("openfile"''', ''' // eventBus._on("openfile"''') \
                .replace(''' eventBus.on("print"''', ''' // eventBus._on("print"''') \
                .replace(''' eventBus.on("download"''', ''' // eventBus._on("download"''')
            fout.write(line)
