...
async function lintRootFile(rootPath: string) {
    ...
    const stdout = await chktexWrapper('root', lw.file.fileUriFromPath(rootPath), rootPath, requiredArgs, undefined)
...
function parseLog(log: string, singleFileOriginalPath?: string, tabSizeArg?: number) {
    ...
    if (singleFileOriginalPath === undefined) {
        ...
        chkTeX.linterDiagnostics.set(lw.file.fileUriFromPath(singleFileOriginalPath), [])
    }
...
function showLinterDiagnostics(linterLog: ChkTeXLogEntry[]) {
    ...
    for (const file in diagsCollection) {
        ...
        if (['.tex', '.bbx', '.cbx', '.dtx'].includes(path.extname(file))) {
            ...
            chkTeX.linterDiagnostics.set(lw.file.fileUriFromPath(file1), diagsCollection[file])
        }
    }
}
