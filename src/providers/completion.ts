...
export class Completer implements vscode.CompletionItemProvider {
    ...
    constructor (extension: Extension) {
        ...
        this.loadDefaultItems().catch(err => this.extension.logger.addLogMessage(`Error reading data: ${err}.`))
    }

    async loadDefaultItems () {
        const defaultEnvs = fs.readFileSync(`${this.extension.extensionRoot}/data/environments.json`, {
            encoding: 'utf8',
        })
        const defaultCommands = fs.readFileSync(`${this.extension.extensionRoot}/data/commands.json`, {
            encoding: 'utf8',
        })
        const defaultLaTeXMathSymbols = fs.readFileSync(
            `${this.extension.extensionRoot}/data/packages/latex-mathsymbols_cmd.json`,
            { encoding: 'utf8' },
        )
    ...
    provideCompletionItems (
        ...
        return new Promise((resolve, _reject) => {
            ...
            if (
                ...
                currentLine[position.character - 1] === '\\' &&
                currentLine[position.character - 2] === '\\'
            ) {
                resolve()
            ...
            if (this.command.specialBrackets && this.command.specialBrackets.hasOwnProperty(invokeChar)) {
                if (position.character > 1 && currentLine[position.character - 2] === '\\') {
                    const mathSnippet = { ...this.command.specialBrackets[invokeChar] }
                    if (
                        vscode.workspace.getConfiguration('editor', document.uri).get('autoClosingBrackets') &&
                        (currentLine.length > position.character &&
                            [')', ']', '}'].indexOf(currentLine[position.character]) > -1)
                    ) {
