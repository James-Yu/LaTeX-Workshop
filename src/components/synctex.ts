...
import { Block, parseSyncTex, PdfSyncObject, SyncTexJsError } from '../lib/synctexjs'
import { SyncTeXRecordBackward, SyncTeXRecordForward } from './locator'
...
const iconvLiteSupportedEncodings = [
    'utf8',
    'utf16le',
    'UTF-16BE',
    'UTF-16',
...
class Rectangle {
    ...
    distanceXY (x: number, y: number) : number {
        return Math.sqrt(
            Math.pow(Math.min(Math.abs(this.bottom - y), Math.abs(this.top - y)), 2) +
                Math.pow(Math.min(Math.abs(this.left - x), Math.abs(this.right - x)), 2),
        )
    }
