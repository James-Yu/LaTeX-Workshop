...
texdoc、文字数カウント、数式プレビューパネル、自動 lint 実行など、追加のツールや UI 機能に依存する利便機能を削除または無効化しています。残っている formatter/linter の実行パスでも、workspace 設定で上書きされた実行ファイルは確認ダイアログなしには実行されません。
