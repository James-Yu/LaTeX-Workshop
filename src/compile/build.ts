...
function autoBuild(file: string, type: 'onFileChange' | 'onSave', bibChanged: boolean = false) {
    const configuration = vscode.workspace.getConfiguration('latex-workshop', lw.file.fileUriFromPath(file))
...
function canAutoBuild(): boolean {
    const configuration = vscode.workspace.getConfiguration('latex-workshop', lw.root.file.path ? lw.file.fileUriFromPath(lw.root.file.path) : undefined)
...
async function build(skipSelection: boolean = false, rootFile: string | undefined = undefined, languageId: string | undefined = undefined, recipe: string | undefined = undefined) {
    ...
    const workspace = rootFile ? lw.file.fileUriFromPath(rootFile) : activeEditor.document.uri
...
function spawnProcess(step: Step): ProcessEnv {
    const configuration = vscode.workspace.getConfiguration('latex-workshop', step.rootFile ? lw.file.fileUriFromPath(step.rootFile) : undefined)
...
async function afterSuccessfulBuilt(lastStep: Step, skipped: boolean) {
    ...
    const configuration = vscode.workspace.getConfiguration('latex-workshop', lw.file.fileUriFromPath(lastStep.rootFile))
