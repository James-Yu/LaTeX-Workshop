...
This document is largely historical and upstream-oriented. In TeX Workspace Secure, the internal PDF viewer server, browser viewer workflow, and SyncTeX integration are disabled in the secure build, so the architecture described below is no longer the active runtime path.
